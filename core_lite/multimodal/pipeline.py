...
Supports all input modalities:
    text, voice, image, screenshot, document, structured_data,
    bundle, workflow_artifact, repo_state, llm_output, unknown

All 13 pipeline steps per the StegVerse Multimodal Input Strategy.
"""
...
MODALITY_RULES = {
    "text": {
        ...
        "review_required_if": [],
        "notes": "Declare scope and purpose. Check for private context.",
    },
    "voice": {
        "sensitive_by_default": True,
        "review_required_if": ["background_capture", "speaker_unverified"],
        "notes": "Voice is identity-adjacent but not identity-proof. Speaker not verified by default.",
    },
    "image": {
        "sensitive_by_default": False,
        "review_required_if": ["contains_people"],
        "notes": "Images may contain people, locations, private documents.",
    },
    "screenshot": {
        "sensitive_by_default": True,
        "review_required_if": ["credentials", "tokens", "private_messages", "financial_data"],
        "notes": "Screenshots require review if they contain credentials, tokens, or private data.",
    },
    "document": {
        "sensitive_by_default": False,
        "review_required_if": ["pii", "legal", "medical", "financial"],
        "notes": "Documents may contain PII, legal, medical, or financial content.",
    },
    "structured_data": {
        "sensitive_by_default": False,
        "review_required_if": ["pii_columns", "financial_columns", "health_columns"],
        "notes": "Check for PII, financial, or health data in columns.",
    },
    "bundle": {
        "sensitive_by_default": False,
        "review_required_if": ["missing_manifest", "hash_mismatch"],
        "notes": "Bundle requires manifest and path validation.",
    },
    "llm_output": {
        "sensitive_by_default": False,
        "review_required_if": ["operator_not_reviewed"],
        "notes": "LLM output is reasoning evidence, not authority. Cannot authorize mutation.",
    },
    "unknown": {
        "sensitive_by_default": True,
        "review_required_if": ["always"],
        "notes": "Unknown input type always requires review.",
    },
}
...
class MultimodalPipeline:
    """
    Full 13-step multimodal input pipeline.

    Enforces:
    - Voice is identity-adjacent but not identity-proof
    - Screenshots are sensitive by default
    - LLM output is reasoning evidence, not authority
    - Unknown fails to REVIEW_REQUIRED
    """
    ...
    def run(self, manifest: MultimodalInputManifest) -> PipelineResult:
        ...
        # Step 2 — Store/reference in KnowledgeVault
        ...
        result.steps.append(self._step(4, "classify", "ok",
                                        input_type=manifest.input_type,
                                        rules=modality_rules.get("notes", "")))

        # Step 5 — Identity context
        identity_status = self._check_identity(manifest)
        result.steps.append(self._step(5, "identity_context", identity_status,
                                        identity_context=manifest.identity_context,
                                        speaker_verified=manifest.speaker_verified))

        # Step 6 — Scope
        if not manifest.target_scope:
            manifest.target_scope = "core-lite"
        result.steps.append(self._step(6, "scope_declaration", "ok",
        ...
        result.steps.append(self._step(7, "privacy_consent", privacy_status,
        ...
        result.steps.append(self._step(8, "intended_use", "ok",
        ...
        if precheck_issues:
            receipt = self.recorder.record(
                actor=self.entity, stage=self.stage, gate="multimodal_pipeline",
        ...
        result.steps.append(self._step(10, "cge_admissibility", "ok",
        ...
        result.receipt_hash = receipt.receipt_hash
        result.steps.append(self._step(11, "receipt", "ok",
        ...
        result.steps.append(self._step(13, "stop_enforcement", "ok",
                                        admitted=admitted,
                                        stop_condition=manifest.stop_condition))
    ...
    def _kv_path(self, manifest: MultimodalInputManifest) -> Path:
        return (self.repo_root / "vault_template" / "manifests" /
                f"{manifest.input_id or 'input'}_manifest.json")

    def _write_kv_manifest(self, manifest: MultimodalInputManifest, path: Path):
    ...
    def _classify_privacy(self, manifest: MultimodalInputManifest,
                           rules: dict) -> tuple[str, list]:
        notes = []
